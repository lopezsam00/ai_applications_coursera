...
def main():
    try:
        ...
        food_items = load_food_data("files/FoodDataSet.json")
        ...
        print(f"❌ Error initializing advanced search system: {err}")
...
def perform_basic_search(collection):
    ...
    display_search_results(results, "Basic Similarity Search Results")
...
def perform_cuisine_filtered_search(collection):
    ...
    if cuisine_choice.isdigit():
        ...
        if 0 <= idx < len(cuisines):
            cuisine_filter = cuisine_filter["idx"]
    else:
